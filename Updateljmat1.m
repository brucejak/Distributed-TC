function [new_ljmat,cutlinkpower] = Updateljmat(old_ljmat,neiborloc,community,commu_DM,G,ALL_DM,iniM)
    ...
    for i=1:1:community_num
        if ~isempty(neiborloc{i})
           ...
           % linkloc2=yy(1);
            ...
            cutlinkpower(th,5)=sum(iniM(community{i}));
            cutlinkpower(th,6)=sum(iniM(community{neiborloc{i}}));
    ...
    for i=1:1:community_num
        if ~isempty(neiborloc{i})
            %[linkloc1,linkloc2]=maxdisttwinsloc(community{i},community{neiborloc{i}},ALL_DM);
             linkloc1 = community{i}(1);
            linkloc2 = community{neiborloc{i}}(1);
            %linkloc1 = community{i};
           % linkloc2 = community{neiborloc{i}};
            %linkloc1=linklocCell{i,neiborloc{i}}(1);
            %linkloc2=linklocCell{i,neiborloc{i}}(2);
            %old_ljmat(linkloc1,linkloc2)=1;
            %old_ljmat(linkloc2,linkloc1)=1;
            ...
            %cutlinkpower(th,5)=1;
            %cutlinkpower(th,6)=1;
            cutlinkpower(th,5)=iniM(community{i});
            cutlinkpower(th,6)=iniM(community{neiborloc{i}});
            cutlinkpower(th,7)=commu_DM(i,neiborloc{i});
            th=th+1;
        end
    end
